...
prob_transmission_m2 <- 0.358
prob_clustering_m2 <-0.5

r0_tracing_list_t_m2<- c()

N <- number_of_nodes_constant2(population_generations)
contacts_fixed <- creating_fixed_contacts_constant2(population_generations)
...
for(prob_tracing in t_interval){
  ...
  for(j in 1:M){
    contacts_m2 <- creating_contacts_constant2(contacts_fixed, contacts_extra, prob_clustering_m2)
...
#results from table
prob_clustering_m1 <-  0.3744847
binom_size <- round((1.5019^2)/(1.5019-1.2417))
binom_prob <- 1.5019/binom_size
...
for(prob_tracing in t_interval){
  ...
  for(j in 1:M){
    ...
    offspring_distr_m1 <- creating_offspringdistr_binom_m1(binom_size, binom_prob, population_generations)
...
for(prob_tracing in c(0.7,0.8,0.9,1)){
  infection_gens <- c()
...
ggplot(data = combined_df, aes(x = x, y = y, color = dataset)) +
  geom_line(aes(linetype=dataset, color=dataset)) +
  geom_point(data = df_r0_t_m1, aes(x = x, y = y))+
  geom_point(data = df_r0_t_m2, aes(x = x, y = y))+
  scale_color_manual(values = c("Model 1" ='#D60270', "Model 2" = '#0038A8', "no clustering" =  'black'))+
  scale_linetype_manual(values = c("Model 1" ='solid', "Model 2" = 'solid', "no clustering" =  'dashed'))+
  theme_light()+
